...
struct Trajectory
{
    ...
    explicit Trajectory(const uint32_t id, const std::vector<SamplePoint>& points, const int start_index, const int end_index) : id(id), points(points), start_index(start_index), end_index(end_index) {}
...
{
    uint32_t id;
    short start_index = -1; short end_index = -1;

    explicit ReferenceTrajectory(const uint32_t id, const short start_index, const short end_index) : id(id), start_index(start_index), end_index(end_index) {}
};
...
struct std::hash<Trajectory>
{
    std::size_t operator()(const Trajectory& t) const noexcept
    {
        return (
            (hash<uint32_t>()(t.id) ^
            (hash<int>()(t.start_index) << 2) ^
            (hash<int>()(t.end_index) >> 1) ^
            (hash<uint32_t>()(t.points.size()) << 1)) >> 1
        );
    }
};
...
int main()
{
    Trajectory t1(1, std::vector{
        SamplePoint(3, 15.5, 0),
        SamplePoint(5, 15.5, 1),
        SamplePoint(7, 15.5, 2),
